...
fn svc(state: &AppState) -> Arc<CatalogService> {
...
pub async fn list_units_of_measure(
    ...
    let (data, total) = svc(&state)
        .list_units_of_measure(q.limit, q.offset, q.search)
        .await
        .map_err(|e| (StatusCode::from(&e), e.to_string()))?;
...
pub async fn list_unit_conversions(
    _user: CurrentUser,
    State(state): State<AppState>,
    Query(q): Query<ConversionListQuery>,
) -> Result<Json<UnitConversionsListResponse>, (StatusCode, String)> {
    let (data, total) = svc(&state)
        .list_unit_conversions(q.limit, q.offset, q.from_unit_id, q.to_unit_id)
        .await
        .map_err(|e| (StatusCode::from(&e), e.to_string()))?;
...
pub async fn create_catmat_group(
    _user: CurrentUser,
    State(state): State<AppState>,
    Json(p): Json<CreateCatmatGroupPayload>,
) -> Result<(StatusCode, Json<CatmatGroupDto>), (StatusCode, String)> {
    svc(&state)
        .create_catmat_group(p)
        .await
...
pub async fn get_catmat_group(
    _user: CurrentUser,
    State(state): State<AppState>,
    Path(id): Path<Uuid>,
) -> Result<Json<CatmatGroupDto>, (StatusCode, String)> {
    svc(&state)
        .get_catmat_group(id)
        .await
        .map(Json)
...
pub async fn list_catmat_groups(
    _user: CurrentUser,
    State(state): State<AppState>,
    Query(q): Query<GroupListQuery>,
) -> Result<Json<CatmatGroupsListResponse>, (StatusCode, String)> {
    let (data, total) = svc(&state)
        .list_catmat_groups(q.limit, q.offset, q.search, q.is_active)
        .await
        .map_err(|e| (StatusCode::from(&e), e.to_string()))?;
    Ok(Json(CatmatGroupsListResponse {
        data,
        total,
        limit: q.limit,
        offset: q.offset,
...
pub async fn update_catmat_group(
    _user: CurrentUser,
    State(state): State<AppState>,
    Path(id): Path<Uuid>,
    Json(p): Json<UpdateCatmatGroupPayload>,
) -> Result<Json<CatmatGroupDto>, (StatusCode, String)> {
    svc(&state)
        .update_catmat_group(id, p)
        .await
        .map(Json)
        .map_err(|e| (StatusCode::from(&e), e.to_string()))
}
...
pub async fn delete_catmat_group(
    _user: CurrentUser,
    State(state): State<AppState>,
    Path(id): Path<Uuid>,
) -> Result<StatusCode, (StatusCode, String)> {
    svc(&state)
        .delete_catmat_group(id)
        .await
        .map(|_| StatusCode::NO_CONTENT)
        .map_err(|e| (StatusCode::from(&e), e.to_string()))
}
...
pub async fn get_catmat_tree(
    _user: CurrentUser,
    State(state): State<AppState>,
) -> Result<Json<Vec<CatmatGroupTreeNode>>, (StatusCode, String)> {
    svc(&state)
        .get_catmat_tree()
        .await
        .map(Json)
...
pub async fn create_catmat_class(
    _user: CurrentUser,
    State(state): State<AppState>,
    Json(p): Json<CreateCatmatClassPayload>,
) -> Result<(StatusCode, Json<CatmatClassWithDetailsDto>), (StatusCode, String)> {
    let c = svc(&state)
        .create_catmat_class(p)
        .await
        .map_err(|e| (StatusCode::from(&e), e.to_string()))?;
    svc(&state)
        .get_catmat_class(c.id)
        .await
        .map(|c| (StatusCode::CREATED, Json(c)))
        .map_err(|e| (StatusCode::from(&e), e.to_string()))
}
...
pub async fn get_catmat_class(
    _user: CurrentUser,
    State(state): State<AppState>,
    Path(id): Path<Uuid>,
) -> Result<Json<CatmatClassWithDetailsDto>, (StatusCode, String)> {
    svc(&state)
        .get_catmat_class(id)
        .await
        .map(Json)
...
pub async fn list_catmat_classes(
    _user: CurrentUser,
    State(state): State<AppState>,
    Query(q): Query<ClassListQuery>,
) -> Result<Json<CatmatClassesListResponse>, (StatusCode, String)> {
    let (data, total) = svc(&state)
        .list_catmat_classes(q.limit, q.offset, q.search, q.group_id, q.is_active)
        .await
        .map_err(|e| (StatusCode::from(&e), e.to_string()))?;
    Ok(Json(CatmatClassesListResponse {
        data,
        total,
        limit: q.limit,
        offset: q.offset,
...
pub async fn update_catmat_class(
    _user: CurrentUser,
    State(state): State<AppState>,
    Path(id): Path<Uuid>,
    Json(p): Json<UpdateCatmatClassPayload>,
) -> Result<Json<CatmatClassWithDetailsDto>, (StatusCode, String)> {
    let _ = svc(&state)
        .update_catmat_class(id, p)
        .await
        .map_err(|e| (StatusCode::from(&e), e.to_string()))?;
    svc(&state)
        .get_catmat_class(id)
...
pub async fn delete_catmat_class(
    _user: CurrentUser,
    State(state): State<AppState>,
    Path(id): Path<Uuid>,
) -> Result<StatusCode, (StatusCode, String)> {
    svc(&state)
        .delete_catmat_class(id)
        .await
        .map(|_| StatusCode::NO_CONTENT)
        .map_err(|e| (StatusCode::from(&e), e.to_string()))
}
...
pub async fn create_catmat_pdm(
    _user: CurrentUser,
    State(state): State<AppState>,
    Json(p): Json<CreateCatmatPdmPayload>,
) -> Result<(StatusCode, Json<CatmatPdmWithDetailsDto>), (StatusCode, String)> {
    let c = svc(&state)
        .create_catmat_pdm(p)
        .await
        .map_err(|e| (StatusCode::from(&e), e.to_string()))?;
...
pub async fn get_catmat_pdm(
    _user: CurrentUser,
    State(state): State<AppState>,
    Path(id): Path<Uuid>,
) -> Result<Json<CatmatPdmWithDetailsDto>, (StatusCode, String)> {
    svc(&state)
        .get_catmat_pdm(id)
        .await
        .map(Json)
...
pub async fn list_catmat_pdms(
    _user: CurrentUser,
    State(state): State<AppState>,
    Query(q): Query<PdmListQuery>,
) -> Result<Json<CatmatPdmsListResponse>, (StatusCode, String)> {
    let (data, total) = svc(&state)
        .list_catmat_pdms(q.limit, q.offset, q.search, q.class_id, q.is_active)
        .await
        .map_err(|e| (StatusCode::from(&e), e.to_string()))?;
    Ok(Json(CatmatPdmsListResponse {
        data,
        total,
        limit: q.limit,
        offset: q.offset,
...
pub async fn update_catmat_pdm(
    _user: CurrentUser,
    State(state): State<AppState>,
    Path(id): Path<Uuid>,
    Json(p): Json<UpdateCatmatPdmPayload>,
) -> Result<Json<CatmatPdmWithDetailsDto>, (StatusCode, String)> {
    let _ = svc(&state)
        .update_catmat_pdm(id, p)
        .await
        .map_err(|e| (StatusCode::from(&e), e.to_string()))?;
    svc(&state)
        .get_catmat_pdm(id)
...
pub async fn delete_catmat_pdm(
    _user: CurrentUser,
    State(state): State<AppState>,
    Path(id): Path<Uuid>,
) -> Result<StatusCode, (StatusCode, String)> {
    svc(&state)
        .delete_catmat_pdm(id)
        .await
        .map(|_| StatusCode::NO_CONTENT)
        .map_err(|e| (StatusCode::from(&e), e.to_string()))
}
...
pub async fn create_catmat_item(
    _user: CurrentUser,
    State(state): State<AppState>,
    Json(p): Json<CreateCatmatItemPayload>,
) -> Result<(StatusCode, Json<CatmatItemWithDetailsDto>), (StatusCode, String)> {
    let i = svc(&state)
        .create_catmat_item(p)
        .await
        .map_err(|e| (StatusCode::from(&e), e.to_string()))?;
    svc(&state)
        .get_catmat_item(i.id)
        .await
...
pub async fn get_catmat_item(
    _user: CurrentUser,
    State(state): State<AppState>,
    Path(id): Path<Uuid>,
) -> Result<Json<CatmatItemWithDetailsDto>, (StatusCode, String)> {
    svc(&state)
        .get_catmat_item(id)
        .await
        .map(Json)
...
pub async fn list_catmat_items(
    _user: CurrentUser,
    State(state): State<AppState>,
    Query(q): Query<CatmatItemListQuery>,
) -> Result<Json<CatmatItemsListResponse>, (StatusCode, String)> {
    let (data, total) = svc(&state)
        .list_catmat_items(
            q.limit,
            q.offset,
        ...
        .await
        .map_err(|e| (StatusCode::from(&e), e.to_string()))?;
    Ok(Json(CatmatItemsListResponse {
        data,
        total,
        limit: q.limit,
        offset: q.offset,
...
pub async fn update_catmat_item(
    _user: CurrentUser,
    State(state): State<AppState>,
    Path(id): Path<Uuid>,
    Json(p): Json<UpdateCatmatItemPayload>,
) -> Result<Json<CatmatItemWithDetailsDto>, (StatusCode, String)> {
    let _ = svc(&state)
        .update_catmat_item(id, p)
        .await
        .map_err(|e| (StatusCode::from(&e), e.to_string()))?;
    svc(&state)
        .get_catmat_item(id)
...
pub async fn delete_catmat_item(
    _user: CurrentUser,
    State(state): State<AppState>,
    Path(id): Path<Uuid>,
) -> Result<StatusCode, (StatusCode, String)> {
    svc(&state)
        .delete_catmat_item(id)
        .await
        .map(|_| StatusCode::NO_CONTENT)
        .map_err(|e| (StatusCode::from(&e), e.to_string()))
}
...
pub async fn create_catser_section(
    _user: CurrentUser,
    State(state): State<AppState>,
    Json(p): Json<CreateCatserSectionPayload>,
) -> Result<(StatusCode, Json<CatserSectionDto>), (StatusCode, String)> {
    svc(&state)
        .create_catser_section(p)
        .await
...
pub async fn get_catser_section(
    _user: CurrentUser,
    State(state): State<AppState>,
    Path(id): Path<Uuid>,
) -> Result<Json<CatserSectionWithDetailsDto>, (StatusCode, String)> {
    svc(&state)
        .get_catser_section(id)
        .await
        .map(Json)
        .map_err(|e| (StatusCode::from(&e), e.to_string()))
}
...
pub async fn list_catser_sections(
    _user: CurrentUser,
    State(state): State<AppState>,
    Query(q): Query<SectionListQuery>,
) -> Result<Json<CatserSectionsListResponse>, (StatusCode, String)> {
    let (data, total) = svc(&state)
        .list_catser_sections(q.limit, q.offset, q.search, q.is_active)
        .await
        .map_err(|e| (StatusCode::from(&e), e.to_string()))?;
    Ok(Json(CatserSectionsListResponse {
        data,
        total,
        limit: q.limit,
        offset: q.offset,
...
pub async fn update_catser_section(
    _user: CurrentUser,
    State(state): State<AppState>,
    Path(id): Path<Uuid>,
    Json(p): Json<UpdateCatserSectionPayload>,
) -> Result<Json<CatserSectionDto>, (StatusCode, String)> {
    svc(&state)
        .update_catser_section(id, p)
        .await
        .map(Json)
        .map_err(|e| (StatusCode::from(&e), e.to_string()))
}
...
pub async fn delete_catser_section(
    _user: CurrentUser,
    State(state): State<AppState>,
    Path(id): Path<Uuid>,
) -> Result<StatusCode, (StatusCode, String)> {
    svc(&state)
        .delete_catser_section(id)
        .await
        .map(|_| StatusCode::NO_CONTENT)
        .map_err(|e| (StatusCode::from(&e), e.to_string()))
}
...
pub async fn get_catser_section_tree(
    _user: CurrentUser,
    State(state): State<AppState>,
) -> Result<Json<Vec<CatserSectionTreeNode>>, (StatusCode, String)> {
    svc(&state)
        .get_catser_section_tree()
        .await
        .map(Json)
        .map_err(|e| (StatusCode::from(&e), e.to_string()))
}
...
pub async fn create_catser_division(
    _user: CurrentUser,
    State(state): State<AppState>,
    Json(p): Json<CreateCatserDivisionPayload>,
) -> Result<(StatusCode, Json<CatserDivisionWithDetailsDto>), (StatusCode, String)> {
    let d = svc(&state)
        .create_catser_division(p)
        .await
        .map_err(|e| (StatusCode::from(&e), e.to_string()))?;
    svc(&state)
        .get_catser_division(d.id)
        .await
...
pub async fn get_catser_division(
    _user: CurrentUser,
    State(state): State<AppState>,
    Path(id): Path<Uuid>,
) -> Result<Json<CatserDivisionWithDetailsDto>, (StatusCode, String)> {
    svc(&state)
        .get_catser_division(id)
        .await
        .map(Json)
        .map_err(|e| (StatusCode::from(&e), e.to_string()))
}
...
pub async fn list_catser_divisions(
    _user: CurrentUser,
    State(state): State<AppState>,
    Query(q): Query<DivisionListQuery>,
) -> Result<Json<CatserDivisionsListResponse>, (StatusCode, String)> {
    let (data, total) = svc(&state)
        .list_catser_divisions(q.limit, q.offset, q.search, q.section_id, q.is_active)
        .await
        .map_err(|e| (StatusCode::from(&e), e.to_string()))?;
    Ok(Json(CatserDivisionsListResponse {
        data,
        total,
        limit: q.limit,
        offset: q.offset,
...
pub async fn update_catser_division(
    _user: CurrentUser,
    State(state): State<AppState>,
    Path(id): Path<Uuid>,
    Json(p): Json<UpdateCatserDivisionPayload>,
) -> Result<Json<CatserDivisionWithDetailsDto>, (StatusCode, String)> {
    let _ = svc(&state)
        .update_catser_division(id, p)
        .await
        .map_err(|e| (StatusCode::from(&e), e.to_string()))?;
    svc(&state)
        .get_catser_division(id)
...
pub async fn delete_catser_division(
    _user: CurrentUser,
    State(state): State<AppState>,
    Path(id): Path<Uuid>,
) -> Result<StatusCode, (StatusCode, String)> {
    svc(&state)
        .delete_catser_division(id)
        .await
        .map(|_| StatusCode::NO_CONTENT)
        .map_err(|e| (StatusCode::from(&e), e.to_string()))
}
...
pub async fn create_catser_group(
    _user: CurrentUser,
    State(state): State<AppState>,
    Json(p): Json<CreateCatserGroupPayload>,
) -> Result<(StatusCode, Json<CatserGroupDto>), (StatusCode, String)> {
    svc(&state)
        .create_catser_group(p)
        .await
        .map(|g| (StatusCode::CREATED, Json(g)))
        .map_err(|e| (StatusCode::from(&e), e.to_string()))
}
...
pub async fn get_catser_group(
    _user: CurrentUser,
    State(state): State<AppState>,
    Path(id): Path<Uuid>,
) -> Result<Json<CatserGroupDto>, (StatusCode, String)> {
    svc(&state)
        .get_catser_group(id)
        .await
        .map(Json)
...
pub async fn list_catser_groups(
    _user: CurrentUser,
    State(state): State<AppState>,
    Query(q): Query<CatserGroupListQuery>,
) -> Result<Json<CatserGroupsListResponse>, (StatusCode, String)> {
    let (data, total) = svc(&state)
        .list_catser_groups(q.limit, q.offset, q.search, q.division_id, q.is_active)
        .await
        .map_err(|e| (StatusCode::from(&e), e.to_string()))?;
    Ok(Json(CatserGroupsListResponse {
        data,
        total,
        limit: q.limit,
        offset: q.offset,
...
pub async fn update_catser_group(
    _user: CurrentUser,
    State(state): State<AppState>,
    Path(id): Path<Uuid>,
    Json(p): Json<UpdateCatserGroupPayload>,
) -> Result<Json<CatserGroupDto>, (StatusCode, String)> {
    svc(&state)
        .update_catser_group(id, p)
        .await
        .map(Json)
        .map_err(|e| (StatusCode::from(&e), e.to_string()))
}
...
pub async fn delete_catser_group(
    _user: CurrentUser,
    State(state): State<AppState>,
    Path(id): Path<Uuid>,
) -> Result<StatusCode, (StatusCode, String)> {
    svc(&state)
        .delete_catser_group(id)
        .await
        .map(|_| StatusCode::NO_CONTENT)
        .map_err(|e| (StatusCode::from(&e), e.to_string()))
}
...
pub async fn get_catser_tree(
    _user: CurrentUser,
    State(state): State<AppState>,
) -> Result<Json<Vec<CatserGroupTreeNode>>, (StatusCode, String)> {
    svc(&state)
        .get_catser_tree()
        .await
        .map(Json)
...
pub async fn create_catser_class(
    _user: CurrentUser,
    State(state): State<AppState>,
    Json(p): Json<CreateCatserClassPayload>,
) -> Result<(StatusCode, Json<CatserClassWithDetailsDto>), (StatusCode, String)> {
    let c = svc(&state)
        .create_catser_class(p)
        .await
        .map_err(|e| (StatusCode::from(&e), e.to_string()))?;
    svc(&state)
        .get_catser_class(c.id)
        .await
        .map(|c| (StatusCode::CREATED, Json(c)))
        .map_err(|e| (StatusCode::from(&e), e.to_string()))
}
...
pub async fn get_catser_class(
    _user: CurrentUser,
    State(state): State<AppState>,
    Path(id): Path<Uuid>,
) -> Result<Json<CatserClassWithDetailsDto>, (StatusCode, String)> {
    svc(&state)
        .get_catser_class(id)
        .await
        .map(Json)
...
pub async fn list_catser_classes(
    _user: CurrentUser,
    State(state): State<AppState>,
    Query(q): Query<ClassListQuery>,
) -> Result<Json<CatserClassesListResponse>, (StatusCode, String)> {
    let (data, total) = svc(&state)
        .list_catser_classes(q.limit, q.offset, q.search, q.group_id, q.is_active)
        .await
        .map_err(|e| (StatusCode::from(&e), e.to_string()))?;
    Ok(Json(CatserClassesListResponse {
        data,
        total,
        limit: q.limit,
        offset: q.offset,
...
pub async fn update_catser_class(
    _user: CurrentUser,
    State(state): State<AppState>,
    Path(id): Path<Uuid>,
    Json(p): Json<UpdateCatserClassPayload>,
) -> Result<Json<CatserClassWithDetailsDto>, (StatusCode, String)> {
    let _ = svc(&state)
        .update_catser_class(id, p)
        .await
        .map_err(|e| (StatusCode::from(&e), e.to_string()))?;
    svc(&state)
        .get_catser_class(id)
...
pub async fn delete_catser_class(
    _user: CurrentUser,
    State(state): State<AppState>,
    Path(id): Path<Uuid>,
) -> Result<StatusCode, (StatusCode, String)> {
    svc(&state)
        .delete_catser_class(id)
        .await
        .map(|_| StatusCode::NO_CONTENT)
        .map_err(|e| (StatusCode::from(&e), e.to_string()))
}
...
pub async fn create_catser_item(
    _user: CurrentUser,
    State(state): State<AppState>,
    Json(p): Json<CreateCatserItemPayload>,
) -> Result<(StatusCode, Json<CatserItemWithDetailsDto>), (StatusCode, String)> {
    let i = svc(&state)
        .create_catser_item(p)
        .await
        .map_err(|e| (StatusCode::from(&e), e.to_string()))?;
    svc(&state)
        .get_catser_item(i.id)
        .await
        .map(|i| (StatusCode::CREATED, Json(i)))
        .map_err(|e| (StatusCode::from(&e), e.to_string()))
}
...
pub async fn get_catser_item(
    _user: CurrentUser,
    State(state): State<AppState>,
    Path(id): Path<Uuid>,
) -> Result<Json<CatserItemWithDetailsDto>, (StatusCode, String)> {
    svc(&state)
        .get_catser_item(id)
        .await
        .map(Json)
...
pub async fn list_catser_items(
    _user: CurrentUser,
    State(state): State<AppState>,
    Query(q): Query<CatserItemListQuery>,
) -> Result<Json<CatserItemsListResponse>, (StatusCode, String)> {
    let (data, total) = svc(&state)
        .list_catser_items(q.limit, q.offset, q.search, q.class_id, q.is_active)
        .await
        .map_err(|e| (StatusCode::from(&e), e.to_string()))?;
    Ok(Json(CatserItemsListResponse {
        data,
        total,
        limit: q.limit,
        offset: q.offset,
...
pub async fn update_catser_item(
    _user: CurrentUser,
    State(state): State<AppState>,
    Path(id): Path<Uuid>,
    Json(p): Json<UpdateCatserItemPayload>,
) -> Result<Json<CatserItemWithDetailsDto>, (StatusCode, String)> {
    let _ = svc(&state)
        .update_catser_item(id, p)
        .await
        .map_err(|e| (StatusCode::from(&e), e.to_string()))?;
    svc(&state)
        .get_catser_item(id)
...
pub async fn delete_catser_item(
    _user: CurrentUser,
    State(state): State<AppState>,
    Path(id): Path<Uuid>,
) -> Result<StatusCode, (StatusCode, String)> {
    svc(&state)
        .delete_catser_item(id)
        .await
        .map(|_| StatusCode::NO_CONTENT)
        .map_err(|e| (StatusCode::from(&e), e.to_string()))
}
