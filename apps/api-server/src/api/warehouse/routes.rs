...
pub fn warehouse_routes() -> Router<AppState> {
    Router::new()
        ...
        .route(
            "/stock/adjustment",
            post(handlers::register_stock_adjustment),
        )
        .route("/stock/transfer", post(handlers::transfer_stock))
        .route("/stock/{id}", get(handlers::get_warehouse_stock))
        .route(
            "/stock/{id}/maintenance",
            put(handlers::update_stock_maintenance),
        )
        .route(
            "/stock/{id}/block",
            post(handlers::block_material).delete(handlers::unblock_material),
        )
        // Reports
        .route(
            "/reports/stock-value",
            get(reports_handlers::get_stock_value_report),
        )
        .route(
            "/reports/stock-value/detail",
            get(reports_handlers::get_stock_value_detail),
        )
        .route(
            "/reports/consumption",
            get(reports_handlers::get_consumption_report),
        )
        .route(
            "/reports/most-requested",
            get(reports_handlers::get_most_requested_materials),
        )
        .route(
            "/reports/movement-analysis",
            get(reports_handlers::get_movement_analysis),
        )
}
