...
async fn test_get_fueling() {
    ...
    let resp = app
        .api
        .get(&format!("/api/admin/fuelings/{}", id))
        .add_header("Authorization", format!("Bearer {}", app.admin_token))
        .await;
...
async fn test_update_fueling() {
    ...
    let resp = app
        .api
        .put(&format!("/api/admin/fuelings/{}", id))
        .add_header("Authorization", format!("Bearer {}", app.admin_token))
        .json(&json!({
            "odometer_km": 46500,
...
async fn test_delete_fueling() {
    ...
    let resp = app
        .api
        .delete(&format!("/api/admin/fuelings/{}", id))
        .add_header("Authorization", format!("Bearer {}", app.admin_token))
        .await;
...
async fn test_list_fuelings_filter_by_vehicle() {
    ...
    let resp = app
        .api
        .get(&format!("/api/admin/fuelings?vehicle_id={}", vid))
        .add_header("Authorization", format!("Bearer {}", app.admin_token))
        .await;
    assert_eq!(resp.status_code(), StatusCode::OK);
    let list: Value = resp.json();
    for f in list["data"].as_array().unwrap() {
...
async fn test_list_fuelings_filter_by_driver() {
    ...
    let resp = app
        .api
        .get(&format!("/api/admin/fuelings?driver_id={}", did))
        .add_header("Authorization", format!("Bearer {}", app.admin_token))
        .await;
    assert_eq!(resp.status_code(), StatusCode::OK);
    let list: Value = resp.json();
...
async fn test_get_fueling_not_found() {
    ...
    let resp = app
        .api
        .get(&format!("/api/admin/fuelings/{}", fake_id))
        .add_header("Authorization", format!("Bearer {}", app.admin_token))
        .await;
    assert_eq!(resp.status_code(), StatusCode::NOT_FOUND);
}
