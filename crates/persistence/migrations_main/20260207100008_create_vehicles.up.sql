CREATE TABLE vehicles (
    ...
    -- Mechanical component serial numbers
    injection_pump VARCHAR(25),
    gearbox VARCHAR(25),
    differential VARCHAR(25),
    -- Classification (model carries make and category)
    ...
    cost_sharing BOOLEAN NOT NULL DEFAULT false,
    initial_mileage NUMERIC(12,2),
    ...
    CONSTRAINT chk_vehicles_initial_mileage CHECK (initial_mileage IS NULL OR initial_mileage >= 0),
    CONSTRAINT chk_vehicles_fuel_tank_capacity CHECK (fuel_tank_capacity IS NULL OR fuel_tank_capacity > 0),
...
CREATE INDEX idx_vehicles_fleet_code ON vehicles (fleet_code) WHERE is_deleted = false AND fleet_code IS NOT NULL;
